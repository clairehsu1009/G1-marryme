...
public class ReservationController extends HttpServlet {
	...
	/* 新增 */
	private void insert(HttpServletRequest req, HttpServletResponse resp, Map<String, String> responseMsgMap)
...
// ========================insert資料=======================       
		...
		;
		if (StringUtils.isNotBlank(reqMap.get("reservationDate")[0])
				&& StringUtils.isNotBlank(reqMap.get("reservationTime")[0])) {
		...
		req.getRequestDispatcher(ONE_RESERVATION_USER_CONFIRM).forward(req, resp);
	}

	/* 編輯先取出資料 */
	private void getOne(HttpServletRequest req, HttpServletResponse resp, Map<String, String> responseMsgMap)
			throws Exception {
		...
		String action = req.getParameter(ACTION);
		String url = StringUtils.equals(GET_ONE_FOR_UPDATE, action) ? UPDATE_RESERVATION_ITEM_PAGE
				: ONE_RESERVATION_CONFIRM;
		req.getRequestDispatcher(url).forward(req, resp);
	}

	/* 更新預約日期 */
	private void update(HttpServletRequest req, HttpServletResponse resp, Map<String, String> responseMsgMap)
			throws ServletException, IOException {
		Map<String, String[]> reqMap = req.getParameterMap();
	...
	/** 取得狀態為取消的預約時間清單 */
	private void getCancelReservationList(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
