...
<body class="app sidebar-mini rtl">
	...
	<main class="app-content">
		<div class="app-title">
			...
			<form method="post"
				action="<%=request.getContextPath()%>/order/findOrderByMemberId?memberId=${member.memberId}&orderStatus=1"
				style="margin-bottom: 0px;">
				<input type="hidden" name="memberId" value="${member.memberId}" />
				<ul class="app-breadcrumb breadcrumb">
					<li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
					<li class="breadcrumb-item"><a
						href="<%=request.getContextPath()%>/order/findOrderByMemberId?memberId=${member.memberId}&orderStatus=1">我的購買訂單</a></li>
				</ul>
			</form>
		</div>
			...
			<div class="product-tab col-lg-12 col-12">
				<div class="tab-item">
					<ul class="nav" role="tablist">
						<li><a class="${status == 1 ? 'active' : ''}"
							href="<%=request.getContextPath()%>/order/findOrderByMemberId?memberId=${memberId}&orderStatus=1">未出貨</a></li>
						<li><a class="${status == 2 ? 'active' : ''}"
							href="<%=request.getContextPath()%>/order/findOrderByMemberId?memberId=${memberId}&orderStatus=2">已出貨</a></li>
						<li><a class="${status == 4 ? 'active' : ''}"
							href="<%=request.getContextPath()%>/order/findOrderByMemberId?memberId=${memberId}&orderStatus=4">已取消</a></li>
					</ul>
				</div>
					...
					<div class="tab-content">
						<c:forEach items="${orderList}" var="o" varStatus="s1">
							<div class="strip <c:if test="${s1.index==0}">mt50</c:if>">
								...
								<span style="float: right; padding-right: 20px; color: #f55c4b;"
									data-type="openorder">共${o.orderDetails.size()}件商品</span>
							</div>
								...
								<table class="table">
									<thead class="thead">
										<tr>
											...
											<c:if
												test="${o.orderStatus == 2} or ${o.orderStatus == 3} or ${o.orderStatus == 4}">
												<th scope="col">操作</th>
											</c:if>
										</tr>
									</thead>
									<tbody>
										<c:forEach var="orderDetails" items="${o.orderDetails}"
											...
											<tr>
												<td><img
													src="<%=request.getContextPath()%>/ProductShowPhoto?productId=${orderDetails.productId}"
												...
												<c:if test="${status.index == 0}">
													<td rowspan=${o.orderDetails.size() }><c:if
